...
let make_controller (v: 'a) : 'a value_controller =
  let listeners = {contents = []} in 
  let values = {contents = v} in 
  {add_change_listener = 
  (fun x -> listeners.contents <- (listeners.contents@[x]));
  get_value = (fun () -> values.contents);
  change_value = (fun x-> values.contents<-x; 
  let l1 = listeners.contents in
  (*helper to loop through everything in the list *)
  let rec helper (l:('a-> unit) list) = 
  begin match l with 
  |[] -> () 
  |x::acc -> x values.contents; helper acc
  end
  in
  helper l1)
  }
...
(** TODO: Finally, we can assemble a checkbox widget by re-using our previously
    ...
    *)
(*Implementation for a checkbox *) 
let checkbox (init: bool) (s: string) : widget * bool value_controller =
  let lbl, c= label s in 
  let c2= make_controller init in 
  let (_,y) = lbl.size() in  
  let draw_canvas (g:Gctx.gctx): unit =
    if c2.get_value() then (
    Gctx.draw_line g (0,0) (y-1,y-1); 
    Gctx.draw_line g (y-1,0) (0,y-1)) else () in
  let c, l2 = canvas (y,y) draw_canvas in
  let h, nc = notifier (hpair lbl c) in
  let label_change = mouseclick_listener (fun () -> if c2.get_value()
  then update_value (c2)(fun x->false) else update_value(c2)(fun x->true)) in
  nc.add_event_listener label_change;
  (h, c2)
...
(* * This is like a canvas that has a label and repaints itself everytime *) 
(* * mouse is clicked down or dragged *)
let slider (s:string)(i:int) : widget* int value_controller =
let c2 = make_controller i in
let c3, _ = label s in
let change_canvas (g: Gctx.gctx) (e:Gctx.event): unit =
if (Gctx.event_type e = MouseDrag|| Gctx.event_type e= MouseDown) then
let x,y = Gctx.event_pos e g in 
if x<80&&y<11 then update_value c2 (fun s-> x) else () in
let paint_canvas (g: Gctx.gctx) : unit = 
Gctx.fill_rect g (0,0) (c2.get_value(), 10) in
let c, y2= canvas (80,11) paint_canvas in
let c3, y3 = notifier (border((hpair c (hpair (space (10,11)) c3)))) in
y3.add_event_listener change_canvas;
(c3, c2)
