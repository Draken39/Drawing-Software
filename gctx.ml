...
(**Thickness *) 
type width = {thickness:int} 

let normal: width = {thickness = 2} 

let thick : width = {thickness = 10}
...
type gctx = {
  ...
  thickness: width (** the current thickness *)
...
(** Produce a new Gctx.t with a different pen color and thickness *)
...
let set_graphics_state (gc: gctx) : unit =
  ...
  Graphics.set_line_width t.thickness
...
(**create a gctx with a certain width *)
let with_thickness (g: gctx) (t: width): gctx = 
{g with thickness = t}
